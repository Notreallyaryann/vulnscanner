    ...
    <div className="min-h-screen bg-[#FBFBFC] text-[#1D1D1F] selection:bg-[#D4380D] selection:text-white font-sans antialiased">
      ...
      <header className="fixed top-6 left-1/2 -translate-x-1/2 w-[90%] max-w-4xl h-14 bg-[#FFFFFF]/85 backdrop-blur-md border border-[#E5E5EA] rounded-full shadow-sm flex items-center justify-between px-6 z-50">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-[#86868B]">
          ...
          <button onClick={() => router.push("/github")} className="flex items-center gap-1.5 hover:text-[#D4380D] transition-colors cursor-pointer">
      ...
      <section className="pt-32 pb-20 px-6 max-w-6xl mx-auto flex flex-col items-center text-center">
        ...
        {/* Scan Mode Tabs */}
        {!isScanning && (
          <div className="flex items-center gap-1 p-1 bg-[#F2F2F7] rounded-full mb-4">
            <button className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-white shadow-sm text-xs font-bold text-[#1D1D1F] transition-all">
              <Globe className="w-3.5 h-3.5 text-[#D4380D]" /> Scan Live URL
            </button>
            <button
              onClick={() => router.push("/github")}
              className="flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold text-[#86868B] hover:text-[#1D1D1F] hover:bg-white/60 transition-all cursor-pointer"
            >
              <GithubIcon className="w-3.5 h-3.5" /> Scan GitHub Repo
            </button>
          </div>
        )}
