...
const RISKY_PORTS: Record<number, { label: string; severity: "CRITICAL" | "HIGH" | "MEDIUM" | "LOW"; cvss: number; cve: string; reason: string }> = {
  ...
  return new Promise((resolve, reject) => {
    const args = [
      "-Pn",                              // Skip host discovery — treat host as up
                                          // Required: Cloudflare/CDNs block ICMP so nmap
                                          // thinks host is down and skips port scan entirely
...
export async function runNmapScan(
  ...
  try {
    const [xml, cdnName] = await Promise.all([runNmapProcess(host), cdnPromise]);
    ...
    if (!xml || xml.trim().length === 0) {
      log(`⚠️   Nmap: no output received — scan may have timed out or host is unreachable`);
      return findings;
    }
    ...
    if (openPorts.length === 0) {
      ...
      log(`🔍  Nmap: ${openPorts.length} open port(s) found: ${openPorts.map((p) => p.port).join(", ")}`);
